...
class Player(models.Model):
    ...
    furigana = models.CharField(
        "ふりがな", max_length=50, null=True, blank=True
    )  # 選手名(ふりがな)
    ...
    height = models.DecimalField(
        "身長", max_digits=5, decimal_places=2, null=True, blank=True
    )  # 身長（cm）
    weight = models.DecimalField(
        "体重", max_digits=5, decimal_places=2, null=True, blank=True
    )  # 体重（kg）
    is_married = models.BooleanField(
        "結婚", null=True, blank=True
    )  #  結婚しているかどうか
    partner = models.CharField(
        "結婚相手", max_length=50, null=True, blank=True
    )  #  結婚相手
    ...
    throwing_hand = models.ForeignKey(
        HandThrowing,
        verbose_name="利腕",
    ...
    batting_hand = models.ForeignKey(
        HandBatting,
        verbose_name="利腕",
        on_delete=models.RESTRICT,
        null=True,
    ...
    main_position_category = models.ForeignKey(
        PositionCategory,
        verbose_name="ポジション",
        on_delete=models.RESTRICT,
        null=True,
    ...
    wikipedia_parameter = models.CharField(
        "Wikipedia", max_length=250, null=True, blank=True
    )  #  WikipediaURLパラメータ
    usukoi_parameter = models.IntegerField(
        "日本プロ野球記録", null=True, blank=True
    )  # 日本プロ野球記録サイトのパラメータ
...
class PlayerLatestSummary(models.Model):
    ...
    player_furigana = models.CharField(
    ...
    player_weight = models.DecimalField(
        max_digits=5, decimal_places=2, null=True, blank=True
    )  # 体重（kg）
    player_is_married = models.BooleanField(
    ...
    player_wikipedia_parameter = models.CharField(
        max_length=250, null=True, blank=True
    )  #  WikipediaURLパラメータ
    player_usukoi_parameter = models.IntegerField(
        null=True, blank=True
    )  # 日本プロ野球記録サイトのパラメータ
    player_youtube_parameter = models.CharField(
        max_length=250, null=True, blank=True
    )  #  Youtubeパラメータ
    ...
    common_record_salary = models.DecimalField(
